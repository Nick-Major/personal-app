<?php

namespace App\Http\Resources;

use Illuminate\Http\Request;
use Illuminate\Http\Resources\Json\JsonResource;

class BrigadierAssignmentResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        // Безопасно нормализуем даты из связанной коллекции
        $dates = collect($this->assignmentDates ?? [])
            ->pluck('assignment_date')
            ->map(function ($d) {
                if ($d instanceof \Carbon\Carbon) {
                    return $d->format('Y-m-d');
                }
                if (is_string($d) && strlen($d) > 0) {
                    try {
                        return \Carbon\Carbon::parse($d)->format('Y-m-d');
                    } catch (\Throwable $e) {
                        return null;
                    }
                }
                return null;
            })
            ->filter()
            ->values();

        return [
            'id' => $this->id,
            'brigadier' => new UserResource($this->whenLoaded('brigadier')),
            'initiator' => new UserResource($this->whenLoaded('initiator')),
            // Для совместимости: первая дата из списка
            'assignment_date' => $dates->first(),
            // Полный список дат назначения (нормализованный)
            'assignment_dates' => $dates,
            'status' => $this->status,
            'confirmed_at' => $this->confirmed_at,
            'rejected_at' => $this->rejected_at,
            'rejection_reason' => $this->rejection_reason,
            'created_at' => $this->created_at,
            'updated_at' => $this->updated_at,
        ];
    }
}
